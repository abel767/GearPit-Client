...
function AddressManager() {
  ...
  useEffect(() => {
      if (user?.user?._id) {  // Note: Changed to user.user._id based on your Redux structure
          dispatch(fetchAddresses(user.user._id))
              .unwrap()
              .then((result) => {
                  if (!result) {
                      console.error('No result received from fetchAddresses');
                  }
              })
  ...
  const handleAddAddress = async (addressData) => {
      if (user?.user?._id) {
          try {
              ...
              if (result?.success) {
                  dispatch(fetchAddresses(user.user._id)); // Refresh addresses after adding
                  setIsEditing(false);
              } else {
  ...
  const handleUpdateAddress = async (addressData) => {
      if (user?.user?._id && currentAddress) {
          try {
              ...
              dispatch(fetchAddresses(user.user._id)); // Refresh addresses after update
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {addresses.map((address) => (
              <div key={address._id} className="border rounded-lg p-4 bg-white shadow-sm relative">
                ...
                <AddressMenu onEdit={() => startEditing(address)} onDelete={() => handleDeleteAddress(address._id)} />
              </div>
            ))}
          </div>
